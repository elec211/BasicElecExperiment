...
이번 실험에 사용할 주요 부품과 이를 브레드보드에 구현한 주요 회로도는 다음과 같다. 실험 시작 전 혹은 담당교수의 지시사항이 있을 때 실험 준비실로 이동하여 필요한 수량만큼 해당 부품을 가지고 자리로 돌아온다.
...
예비보고서를 작성할 때는 다음 실험의 전반적인 내용을 읽어보면서 어떠한 내용의 실험을 수행할지 예상하며 요약하도록 한다. 실험에 필요한 이론적인 배경지식이 부족한 상태이므로 이해하기 어려운 부분이 있더라도 아는데 까지 최대한 작성하도록 한다.
...
![01_fuse_test](./images/01.jpg '휴즈 테스트')

과전류 등으로 인해 멀티미터 내부 보호용 퓨즈가 손상될 수 있으므로 멀티미터가 정상동작하지 않을 경우 뒷면 커버를 벗긴 뒤 퓨즈를 눈으로 확인하여 끊어졌는지 확인하여 교체하도록 한다. (멀티미터에 따라서 퓨즈가 끊어져도 short test에서 정상으로 확인되므로 육안으로 확인해야 한다.)
...
![02_저항값읽기](./images/02.jpg '저항값 읽기')

칼라코드의 띠가 한족으로 몰려 있기 때문에 시작점과 오차값의 위치를 구분할 수 있다. 하지만 저항 종류에 따라서 한쪽으로 치우쳐 있지 않는 경우가 있는데 이때는 보통 은색, 금색 띠가 오차 값을 표시하는 띠라고 생각하면 된다.
...
![04_브레드보드_연결테스트](./images/04x.jpg '브레드보드 연결성 테스트')
...
![05_브레드보드_연결테스트_short](./images/05x.jpg '브레드보드 short 지점')
...
------------------
![06_브레드보드_연결테스트_open1](./images/06x.jpg '브레드보드 Open지점')
...
------------------
![07_브레드보드_연결테스트_open2](./images/07x.jpg '브레드보드 Open지점')
...
------------------
![08_브레드보드_연결테스트_short_vertical](./images/08x.jpg '브레드보드 전원라인 short 지점')
...
------------------
![09_브레드보드_연결테스트_open_vertical](./images/09x.jpg '브레드보드 Open 지점')
...
![10_브레드보드_저항측정_정상](./images/10x.jpg '브레드보드에 저항 배치 방법')
...
![12_회로구성_전압측정](./images/12x.jpg '전압측정')
...
------------------
![12b_부팅_5V고정출력](./images/12bx.jpg '전원공급기 5V 고정 출력')
...
------------------
![13_회로구성_전압측정](./images/13x.jpg '회로구성 및 전압측정')
...
6. 5V또는 3.5V가 측정될 것이다. (5V 또는 3.3V 여부는 전원공급기 출력포트 옆 스위치를 이용하여 조절가능. 전원공급기 설명섹션 참고)



------------------
![14_전원공급기_고정전압설정](./images/14x.jpg '전원공급기 고정전압 설정')
...
![15_회로구성_전류측정](./images/15x.jpg '회로구성 및 전류측정')
...
------------------
![16_회로구성_전류측정](./images/16x.jpg '회로구성 및 전류측정')
