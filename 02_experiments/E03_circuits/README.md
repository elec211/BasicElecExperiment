...
3. 직렬 저항회로 전체로 흐르는 전류를 측정해본 뒤 등가저항으로 교체한 뒤 전류가 동일함을 실험을 통해 확인한다.
...
브레드보드 및 멀티미터 불량이 간혹 발생하므로 예상과는 다른 값이 측정 될 경우 다른 여분의 멀티미터 등을 사용하여 장비불량 여부를 검증하도록 한다. 실험에 사용할 부품의 여분이 없거나 표기된 부품이 처음부터 없을 경우 가장 비슷한 저항 값을 사용하여도 무방하다.
...
![01_직렬저항회로구성](./images/01x.jpg '직렬저항 회로 구성')
...
![02_전압분배측정](./images/02x.jpg '전압 분배')

멀티미터를 이용하여 각 저항 양단의 전압을 측정해 본다. 전원을 회로에 공급하기 전에 멀티미터를 전압측정모드를 제대로 설정했는지 확인후에 전원을 회로에 공급하도록 한다. 만약 멀티미터 측정 프로브를 병렬로 회로에 연결한 상태에서 멀티미터를 전류측정모드로 변경할 경우 멀티미터 내부 퓨즈가 끊어질 수 있으므로 항상 주의하자. 
...
![03_전압분배측정](./images/03x.jpg '전압분배 현상 측정')
...
![04_전압분배측정](./images/04x.jpg '전압분배 현상 측정')
...
![05_전압분배측정](./images/05x.jpg '전압분배 측정')
...
![06_전압분배측정](./images/06x.jpg '전압분배 측정')
...
![07_직렬회로_전류측정](./images/07x.jpg '직렬저항회로에서 전류측정')
...
![08_직렬회로_전류측정](./images/08x.jpg '직렬저항회로에서 전류측정')
...
![09_직렬회로_등가저항_전류](./images/09x.jpg '직렬저항회로를 등가저항회로로 교체후 전류 측정')
...
![10_병렬회로](./images/10x.jpg '병렬저항 회로')
...
![11_병렬회로_등가저항](./images/11x.jpg '병렬저항회로의 등가저항 계산')
...
병렬 저항회로를 통해 흐르는 전류를 먼저 측정한다. 등가저항 교체한 뒤 전류가 동일한지 비교해본다.

![12_병렬회로_전류](./images/12x.jpg '병렬저항회로에서 전류 분배 현상 측정')
...
![13_병렬회로_등가저항_전류](./images/13x.jpg '병렬저항회로를 등가저항으로 교체한 뒤 전류 측정')
...
![14_병렬회로_전류분배](./images/14x.jpg '병렬저항회로에서 저항값에 따른 전류변화 측정')
...
4. 가변 저항 상단 나사를 드라이버로 조절하여 저항 값이 1kΩ가 되도록 설정해보자. 멀티미터 측정 LCD화면에 500Ω에 가까운 값이 될 때 현재 가변저항값이 1kΩ라고 예상할 수 있다.
...
![15_병렬회로_가변저항_전류분배](./images/15x.jpg '병렬저항회로에서 저항값에 따른 전류분배현상 관찰')
...
![16_병렬회로_가변저항_전류분배](./images/16x.jpg '병렬저항회로에서 저항값에 따른 전류분배현상 관찰')
