...
![01_Isc_측정](./images/01x.jpg '터미널단에서 short circuit 전류 측정')
...
![02_Voc_측정](./images/02x.jpg '터미널단에서 open circuit 전압 측정')
...
![03_Rth_교체후_측정](./images/03x.jpg '예측된 Rth로 교체한 뒤 전류를 다시 측정')
...
![04_등가회로_영향](./images/04x.jpg '등가회로로 변환한 후 나머지 회로에 영향을 주는지 측정 위한 회로 구성')
...
![05_등가회로_영향측정](./images/05x.jpg '등가회로로 변환하기 전 회로에 대해 전류 측정해두기')
...
![06_등가회로_변환후_영향측정](./images/06x.jpg '등가회로로 변환한 후 회로의 전류 변화를 측정')
...
아래 그림과 같은 임의의 회로 예제에 대해 테브난 등가회로로 변환하는 과정을 살펴보자. 변환대상이 되는 회로의 출력단인 load terminal을 기준으로 Isc 및 Voc를 측정함으로써 내부저항을 결정할 수 있다. 먼저 Isc는 아래 오른쪽 그림과 같이 terminal단을 short시킨 뒤 흐르는 전류를 말한다.

![07_3_등가회로변환예](./images/07_3.jpg '테브난 등가회로 변환과정 예: Isc 측정')

Isc를 결정하기 위해서는 먼저 전원 30V 전원 기준으로 나머지 전체 저항을 계산하여 토탈 전류를 계산한다. 이 전류는 위에서 파란색으로 표시한 것처럼 5K옴과 1K옴으로 병렬 구성된 회로를 거쳐 전류가 분배되며 아래와 같이 최종 Isc를 계산할 수 있다.

![07_4_등가회로변환예](./images/07_4.jpg '테브난 등가회로 변환과정 예: Isc 계산과정')

이제 Voc를 살펴보자. Voc는 등가회로 변환대상이 되는 회로의 terminal단을 open시킨 뒤 측정한 전압이다. 오른쪽 하단에 파란색으로 표시한 것처럼 Voc 측정 지점이 open되어 있으므로 전류가 0이며 1K옴을 거쳐 전압강하가 발생하지 않으므로 Voc 전압은 5K옴 양단 전압과 같다. 

![07_5_등가회로변환예](./images/07_5.jpg '테브난 등가회로 변환과정 예: Voc 측정')

아래와 같은 방법으로 30V 전원관점에서 토탈 저항 및 전류를 계산한 뒤 5K옴 양단 전압 강하를 계산하면 최종 Voc값이 얻어지게 된다.

![07_6_등가회로변환예](./images/07_6.jpg '테브난 등가회로 변환과정 예: Voc 계산')

이렇게 변환 대상이 되는 회로의 terminal단에서 Isc, Voc를 계산하였으므로 최종적으로 내부저항 Rn을 아래와 같이 결정할 수 있다. 이제 Voc와 Rn으로 구성된 테브난 등가회로를 오른쪽과 같이 구성해보자. 이 회로에 대해 똑같은 방법으로 terminal단에서 Voc와 Isc를 측정하면 등가회로 변환 이전 회로에 대한 Isc, Voc와 같음을 알 수 있다. 

![07_7_등가회로변환예](./images/07_7.jpg '테브난 등가회로 변환과정 예: 최종 내부저항 결정 테브난 등가회로로 변환')

이 회로에 대해 Voc, Isc를 실제 측정해보고 최종 등가회로로 변환한 한 뒤 Voc, Isc를 측정하여 값을 비교해보자. 두 회로가 등가라고 볼 수 있는지 생각해본다.
