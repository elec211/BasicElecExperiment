...
5. 전원공급기의 가번전압 출력 크기를 다이얼을 이용하여 조정하면서 회로를 통해 흐르는 전류를 멀티미터를 이용ㅎ여 측정하고 이를 V-I 그래프로 표시해본다. 가로축으로 전압(V), 세로축으로 전류(I)의 그래프로 표시해보았을때 기울기 값과 현재 회로에 사용한 저항크기와의 관계를 생각해본다.
...
4. 가변저항의 상단에 십자로 표신된 부분을 드라이버 등으로 돌려서 A-C 또는 C-B 사이의 저항크기를 가변시킬 수 있다. A-C, C-B 사이의 가변저항의 최대값은 A-B사이에 측정되는 값과 같으며 그 값은 1번,2번에서 서술한 것과 같이 측면부 코드값을 이용하여 계산할 수 있다.  
...
4. 현재 사용한 가번저항은 최대 1kΩ이므로 볼트를 회전시키면 1kΩ보다 작은 저항값이 측정되어야 한다.
...
3. 가변저항으로 흐르는 전류를 측정하기 위해 그림페 표시된 위치에 직렬로 멀티미터를 연결한다.
