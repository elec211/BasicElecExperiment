...
위 그림은 멀티미터를 이용하여 가변저항값을 가지는 두지점사이의 저항을 측정하는 모습을 보여준다. 상단 다이얼을 가장 왼쪽으로 돌리면 0옴에 도달하며 이때 멀티미터의 모드를 전류측정모드로 설정하지 않도록 주의한다. 이때 멀티미터를 이용하여 직렬로 전류를 측정을 하게 되면 내부 퓨즈가 끊어질 가능성이 있으니 주의하자.
