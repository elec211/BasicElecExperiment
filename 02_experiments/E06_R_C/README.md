...
3. 저항-커패시터 직렬회로를 구성한 뒤 사인파를 입력으로 인가하였을 때 커패시터 양단 전압의 지연, 전압감쇄 특징을 오실로스코프로 직접 확인한다. 입력 주파수 가변, 회로의 저항, 커패시터 값을 각각 변화시키면서 커패시터 양단 전압 변화를 측정하여 커패시터의 주파수 특성을 실험을 통해 확인한다.
...
위 식을 적분하게 되면 커패시터 양단 전압의 관점에서 식을 표현할 수 있다. 즉 커패시터로 들어오는 전류를 적분하여 커패시터 전압이 증가되며 전류가 클수록 전압이 빠르게 상승하나 커패시터 용량 C가 클수록 전압 증가속도도 둔화된다.
...
4. 측정된 전압이 표시된다. 입력전압 5V가 모두 커패시터에 인가됨을 확인한다.
...
6. 입력 주파수에 따른 커패시터 양단 전압변화의 관계를 예측해보자.
...
1. 함수발생기 출력을 2kHz, +/-5V 사인파로 설정하고 그림과 같이 함수발생기 출력 신호를 회로의 입력부에 연결한다.
...
4. 기존 100옴 저항보다 더 작은 저항, 더 큰 저항으로 순차적으로 교체하면서 입력 파형 대비 커패시터 양단 전압의 시간지연, 전압감쇄 효과가 어떻게 변하는지 관찰한다. 저항이 크면 커패시터로 공급하는 전류 크기가 줄어들고 커패시터로 공급할 수 있는 전류가 제한되고 결과적으로 전압 상승 속도를 줄인다고 할 수 있는가?
...
4. 신호에 포함된 빠른 주파수 성분을 제거하기 위해서 R, C값을 어떻게 조절하면 되는지 생각해본다. 고주파 성분을 제거하면서도 출력응답 지연현상을 완화할 수 있는 방법이 있는지 조사해본다. (Optional)
