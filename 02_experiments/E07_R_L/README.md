...
자세한 회로 이론은 2학년 이론수업에서 배우기로 하고 여기서는 간략하게 인덕터의 전기적 특성을 미분식으로 간단히 소개한다. 인덕터는 기본적으로 전류의 흐름을 방해하는 소자로 반대로 해석하면 전류의 흐름이 커질수록 더 큰 전압 강하가 일어난다고 할 수 있다. 따라서 그 관계를 아래 수식으로 풀어쓸 수 있다. 전류값이 아무리 커도 시간에 따른 변화가 없다면 인덕터 양단 전압은 0V가 된다. 

![00_BOM](./images/11_1.jpg )

아래 그림과 같이 2mH 인덕터가 주어졌을 때 시간에 따른 전류 변화를 인가할 경우 유도되는 전압을 표시하였다. 시간에 따른 전류변화가 클수록 더 큰 전압이 유도되며 인덕터 용량이 클수록 마찬가지로 더 큰 전압이 유도된다. 커패시터와는 다르게 전류변화가 커질수록 즉 입력 전원의 주파수가 커질수록 전류변화는 커질 것이고 그에 따라 인덕터 양단 전압은 더 큰 전압이 유도된다는 점을 숙지하자.
