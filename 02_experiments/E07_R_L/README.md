...
2. 인덕터에 교류 전압을 인가하여 전압, 전류 값을 측저앻보고 교류 특성을 이해한다.
...
1. 함수발생기의 출력 주파수를 100kHz에서 더 빠른 주파수로 조절하면서 입력신호 대비 인덕터 양단 전압의 위상변화와 전압감쇄 현상의 변화를 관찰해본다. 위상변화와 전압감쇄 효과가 줄어드는가? 커패시터는 고주파 성분 신호를 잘 통과시킨다고 볼 수 있는가?
...
2. 오실로스코프 Probe 1번 채널 케이블을 그림과 같이 전원전압 관찰 지점에 연결한다.
3. 오실로스코프 Probe 2번 채널 케이블을 그림과 같이 인덕터 양단 전압 관찰 지점에 연결한다. 
4. 기존 100옴 저항에서 더 작은저항, 더 큰저항으로 교제하면서 입력 전원 파형 대비 인덕터 양단 전압의 위상변화, 전압감쇄 효과가 어떻게 변하는지 관찰한다. 
